...
agentLightCode = 360010028331 # Código do Agent Light (Altere aqui se o código mudar)
agentFullCode = 360013923451 # Código de Agent Full "Stuff" que consome licença (altere aqui se o código mudar)
funcaoHeads = 360021124831 # Código de Função atribuída aos Heads

# Verificar Login Master na linha 51, precisa ser um Admin

# Autenticador de login HEAD "funcaoHeads"
def autenticador():
...
while True:
    ...
    if confimacao.upper() == 'S':
        ...
        elif temChamado != 0: # Verifica se o agent tem algum chamado diferente de Fechado
        ...
        elif user2Tipo == 'agent' and user2Role_Type == 0: # Se já for um agent full não deixa reber a licença
            print('{} já é Agent Full e não é possível fazer a troca'.format(firstNameU2))


        elif user1Tipo == 'agent' and user1Role_Type == 0: # Se for um agent Light então adiciona a licença
            fazATroca()
